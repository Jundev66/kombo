 ...
 * Cobrar.
 *
 * **Pago mixto de verdad**, que es como se cobra aquí: tres dólares en
 * efectivo y el resto en bolívares por pago móvil. Se van agregando pagos y la
 * pantalla dice siempre cuánto falta.
 */
export function PaymentSheet({
  ...
  // Lo que sobra es VUELTO, no un error: el cliente paga con un billete de 10
  // una cuenta de 7 todos los días.
  ...
  function addPayment(): void {
    ...
    setError(null)
    setReference('')
    // Lo que queda, ya puesto: el siguiente pago casi siempre es el resto.
  ...
  async function charge(): Promise<void> {
    ...
    try {
      // Si nadie agregó pagos a mano, se cobra todo con el método elegido: el
      // caso más común es «efectivo, justo», y no tiene por qué costar dos
      // toques más.
      await onConfirm(
        payments.length > 0
          ? payments
