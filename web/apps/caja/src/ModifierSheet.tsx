 ...
 *
 * Se abre al tocar algo que los tiene, y no deja seguir sin elegir los
 * obligatorios: «¿término de la carne?» no es una pregunta que se pueda saltar
 * y resolver después en la plancha.
 */
export function ModifierSheet({
  ...
  function toggle(group: ModifierGroup, modifier: Modifier): void {
    setChosen((current) => {
      ...
      // Cuando sólo cabe uno, elegir otro SUSTITUYE en vez de rebotar: nadie
      // quiere «primero quita el que pusiste» con un cliente delante.
        ...
        {groups.map((group) => (
          <fieldset key={group.id}>
            <legend className="mb-2 font-medium text-[var(--text-strong)]">
              {group.name}{' '}
              {/* La regla la explica el SERVIDOR: «Elige una opción.» Aquí no
                  se vuelve a redactar, para que no puedan discrepar. */}
